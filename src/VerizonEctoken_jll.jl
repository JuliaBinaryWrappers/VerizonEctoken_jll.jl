...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import VerizonEctoken_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("020a3b16-f9c3-57ee-bc19-dd452accb145"))
platforms = [Pkg.Artifacts.unpack_platform(e, "VerizonEctoken", artifacts_toml) for e in artifacts["VerizonEctoken"]]
...
if best_platform === nothing
    @debug("Unable to load VerizonEctoken; unsupported platform $(triplet(platform_key_abi()))")
else
    # Load the appropriate wrapper
    include(joinpath(@__DIR__, "wrappers", "$(best_platform).jl"))
end

end  # module VerizonEctoken_jll
